def upgrade():
    ...
    for table in map_merge:
        query = sa.text(
            ...
            "and nt.external_code <> '' "
        )
        op.execute(query, {"table_name": table["table_name"], "type_name": table["type_name"]})
...
def downgrade():
    ...
    for table in map_merge:
        ...
        query = sa.text(
            ...
            "where nt.id=aa.object_id "
        )
        op.execute(query, {"table": table["table_name"], "type_name": table["type_name"]})
